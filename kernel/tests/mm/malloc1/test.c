...
/*
 * The memory between ALLOWED_RANGE_MIN and ALLOWED_RANGE_TOP - 1 is not
 * mapped through TLB. The allocated memory blocks should come from
 * this region only.
 */
#define ALLOWED_RANGE_MIN  0x80000000
#define ALLOWED_RANGE_TOP  0xa0000000
...
test_run (void)
{
	...
	for (phaseno = 0; phaseno < sizeof_array (phases); phaseno++)
		printk ("Allocated in phase #%u: %u bytes\n", phaseno + 1,
